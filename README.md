...
Confidence bands (client-side from `min(scoreConf, choiceConf)`): **suggest** ≥ 0.7 (filled Take/Wait/Reach); **lean** ≥ 0.5 (outline `Lean take|wait|reach`); below that “Not sure enough…”. SoftFail stays uncertain. Details: `docs/pick-coach.md`.
...
- **Pick coach** — advisory TypeSafe/Jev with **lean** band (0.5–0.7 outline chips); softFail / unavailable still shows mover/role (and vacated) why; `feat-*` soft-fails may use labeled **Stub** (never as Jev). See `docs/pick-coach.md`.
